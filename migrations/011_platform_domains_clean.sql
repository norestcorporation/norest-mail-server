-- 011_platform_domains_clean.sql
-- Platform-owned domains and proper address reservation system (clean version)
...
-- 2. Update domains status check if needed
...
-- First update existing data to avoid constraint violations
UPDATE addresses SET status = 'CLAIMED' WHERE status = 'active';
UPDATE addresses SET status = 'AVAILABLE' WHERE status = 'inactive' OR status = 'reserved';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_blocked_addresses_unique ON blocked_addresses (domain_id, lower(local_part));
...
-- 10. Insert default platform domain
